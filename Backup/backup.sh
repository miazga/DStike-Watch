#!/bin/bash
# Backup script for DStike Bad Watch firmware

echo "========================================="
echo "DStike Bad Watch Firmware Backup Tool"
echo "========================================="
echo ""
...
BACKUP_FILE="dstike_backup_${TIMESTAMP}.hex"
...
# Run backup command with retries
MAX_RETRIES=2
RETRY_COUNT=0
SUCCESS=0
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    if [ -z "$AVRDUDE_CONF" ]; then
        $AVRDUDE -p atmega32u4 -c avr109 -P $PORT -U flash:r:$BACKUP_FILE:i
    else
        $AVRDUDE -C $AVRDUDE_CONF -p atmega32u4 -c avr109 -P $PORT -U flash:r:$BACKUP_FILE:i
    fi
...
# Success message (already checked in retry loop)
echo ""
echo "========================================="
...
ls -lh $BACKUP_FILE
echo ""
echo "Please store this file in a safe location!"
